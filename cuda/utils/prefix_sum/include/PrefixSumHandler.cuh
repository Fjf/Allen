...
#define PREFIX_SUM_ALGORITHM(EXPOSED_TYPE_NAME, DEPENDENCIES)                                                   \
  struct EXPOSED_TYPE_NAME : public Allen::Algorithm {                                                          \
    ...
    decltype(make_handler(name, prefix_sum_reduce)) handler_reduce {name, prefix_sum_reduce};                   \
    decltype(make_handler(name, prefix_sum_single_block)) handler_single_block {name, prefix_sum_single_block}; \
    decltype(make_handler(name, prefix_sum_scan)) handler_scan {name, prefix_sum_scan};                         \
