...
namespace Muon {
  ...
  __device__ inline void setAtIndex(
    HitsSoA* hitsSoA,
    uint index,
    int tile,
    float x,
  ...
  __device__ inline void setAtIndex(HitsSoA* hitsSoA, uint index, Hit* hit)
  {
    assert(index < Constants::max_numhits_per_event);
  ...
  struct MuonRawToHits {
    ...
    __device__ inline void addCoordsCrossingMap(
    ...
    __device__ inline void makeStripLayouts(unsigned int station, unsigned int region, MuonLayout* layouts) const
    {
