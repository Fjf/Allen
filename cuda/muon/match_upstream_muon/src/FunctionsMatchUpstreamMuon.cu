...
__device__ bool
match(const float& qop, const KalmanVeloState& state, const Muon::HitsSoA& muon_hits_event, const float* magnet_polarity, const MatchUpstreamMuon::MuonChambers& MuCh, const MatchUpstreamMuon::SearchWindows& Windows)
{
  ...
  for (auto it = MuCh.firstOffsets[tt-1]; it < MuCh.firstOffsets[tt]; it++) {
    ...
    for (int i = 0; i < muon_hits_event.number_of_hits_per_station[ist]; ++i) {
      ...
      if ((muon_hits_event.x[i_h] > std::get<1>(firstWindow)) || (muon_hits_event.x[i_h] < std::get<0>(firstWindow)) || (muon_hits_event.y[i_h] > std::get<3>(firstWindow)) || (muon_hits_event.y[i_h] < std::get<2>(firstWindow))) continue;
...
{
  return kickScale / std::fabs(dtx) + kickOffset;
}



__device__ float dtx(const float& qop)
{

  return kickScale / (std::abs(1.f / qop) - kickOffset);
}
  ...
  )
{
  ...
  std::tuple<float,float,float,float> Window =stationWindow(state, i_station, magnet_hit, slope, muon_hits_event.z[i_station],Windows);
  ...
  for (int i = 0; i < muon_hits_event.number_of_hits_per_station[i_station]; ++i) {
    ...
    if ((muon_hits_event.x[i_h] > std::get<1>(Window)) || (muon_hits_event.x[i_h] < std::get<0>(Window)) || (muon_hits_event.y[i_h] > std::get<3>(Window)) || (muon_hits_event.y[i_h] < std::get<2>(Window))) continue;
...
__device__ std::pair<float,float> fit_linearX(const MatchUpstreamMuon::Hit magnet_hit,
