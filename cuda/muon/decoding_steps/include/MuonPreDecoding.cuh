...
namespace muon_pre_decoding {
  ...
  struct muon_pre_decoding_t : public DeviceAlgorithm, Parameters {
      ...
      cudaEvent_t&) const
    {
      ...
      using function_t = decltype(global_function(muon_pre_decoding));
      function_t function = runtime_options.mep_layout ? function_t {muon_pre_decoding_mep} : function_t {muon_pre_decoding};
