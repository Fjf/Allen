  ...
  const float* dev_inv_clus_res)
{
  ...
  SciFi::Hits scifi_hits(parameters.dev_scifi_hits, total_number_of_scifi_hits, &scifi_geometry, dev_inv_clus_res);
  const SciFi::HitCount scifi_hit_count {parameters.dev_scifi_hit_count, event_number};
  ...
  for (uint i = threadIdx.x; i < number_of_tracks_event; i += blockDim.x) {
    ...
    SciFi::Consolidated::Hits consolidated_hits =
      scifi_tracks.get_hits(parameters.dev_scifi_track_hits, i, &scifi_geometry, dev_inv_clus_res);
