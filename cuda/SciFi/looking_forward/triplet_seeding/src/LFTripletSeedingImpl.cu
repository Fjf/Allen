  ...
  const uint number_of_seeds)
{
    ...
    for (int j = 0; j < l2_size; ++j) {
      ...
      for (int k = 0; k < l1_size; ++k) {
        ...
        if (chi2 < LookingForward::chi2_max_triplet_single) {
          best_combined.push_back(CombinedTripletValue{chi2,
                                  (int16_t) i,
                                  (int16_t) k,
                                  (int16_t) j});
        }
  ...
  //       if (best_k != -1) {
  //         best_combined.push_back(CombinedTripletValue{best_chi2,
  //                                 (int16_t)(i * LookingForward::tile_size + (best_k % LookingForward::tile_size)),
  //                                 (int16_t) h1_rel,
  //                                 (int16_t)(j * LookingForward::tile_size + (best_k / LookingForward::tile_size))});
  //       }
  ...
  std::sort(best_combined.begin(), best_combined.end(), [] (const CombinedTripletValue& a, const CombinedTripletValue& b) {
