  ...
  if (starting_it < last) {
    ...
    for (uint it_number = 0; it_number < number_of_iterations; ++it_number) {
      ...
      store_sorted_cluster_reference_v4(
        ...
        correctedMat,
        ch,
        shared_mat_offsets[correctedMat - SciFi::Constants::n_consecutive_raw_banks * SciFi::Constants::n_mats_per_consec_raw_bank],
  ...
  const char* scifi_geometry)
{
  ...
  for (uint i = SciFi::Constants::n_consecutive_raw_banks + threadIdx.x; i < n_scifi_banks;
    ...
    auto const raw_bank = MEP::raw_bank<SciFiRawBank>(parameters.dev_scifi_raw_input, parameters.dev_scifi_raw_input_offsets, selected_event_number, i);
    pre_decode_raw_bank_v4(hit_count,
                           geom,
                           raw_bank,
                           i,
