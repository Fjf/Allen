...
namespace Associate {
  namespace Consolidated {
    ...
    struct EventTable {
    private:
      typename ForwardType<T>::uint_t* m_base_pointer;
      const uint m_event_tracks_offset;
      const uint m_total_number;
      const uint m_size;

    public:
      __host__ __device__ EventTable(
        typename ForwardType<T>::uint_t* base_pointer,
    ...
    public:
      __host__ __device__ Table(typename ForwardType<T>::char_t* base_pointer, const uint total_number) :
        m_base_pointer(reinterpret_cast<typename ForwardType<T>::uint_t*>(base_pointer)), m_total_number(total_number)
      {}
      ...
      {
        return *reinterpret_cast<typename ForwardType<T>::float_t*>(m_base_pointer + 1);
      }
      ...
      {
        return *reinterpret_cast<typename ForwardType<T>::float_t*>(m_base_pointer + 2 + m_total_number);
      }
      ...
      __host__ __device__ EventTable<typename ForwardType<T>::char_t>
