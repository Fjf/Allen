  ...
  uint* dev_shifted_atomics_velo)
{
  ...
  for (uint h1_rel_index = threadIdx.x; h1_rel_index < number_of_hits_h1; h1_rel_index += blockDim.x) {
    ...
    if (best_fit < Configuration::velo_search_by_triplet::max_scatter_seeding) {
      ...
      const auto trackP =
        atomicAdd(dev_shifted_atomics_velo + 1, 1) & Configuration::velo_search_by_triplet::ttf_modulo_mask;
      ...
      const auto ttfP =
        atomicAdd(dev_shifted_atomics_velo + 2, 1) & Configuration::velo_search_by_triplet::ttf_modulo_mask;
