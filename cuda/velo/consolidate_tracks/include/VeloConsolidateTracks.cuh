  ...
  struct velo_consolidate_tracks_t : public DeviceAlgorithm, Arguments {
      ...
      const HostBuffers& host_buffers) const
    {
      set_size<dev_velo_track_hits_t>(
        manager, value<host_accumulated_number_of_hits_in_velo_tracks_t>(manager) * sizeof(Velo::Hit));
    ...
    {
      function(dim3(value<host_number_of_selected_events_t>(manager)), block_dimension(), cuda_stream)(Arguments {
      ...
      cudaCheck(cudaMemsetAsync(
        offset<dev_accepted_velo_tracks_t>(manager), 1, size<dev_accepted_velo_tracks_t>(manager), cuda_stream));
      ...
      if (runtime_options.do_check) {
        ...
        cudaCheck(cudaMemcpyAsync(
          ...
          offset<dev_atomics_velo_t>(manager),
          size<dev_atomics_velo_t>(manager),
        ...
        cudaCheck(cudaMemcpyAsync(
          ...
          offset<dev_velo_track_hit_number_t>(manager),
          size<dev_velo_track_hit_number_t>(manager),
          cudaMemcpyDeviceToHost,
          cuda_stream));
        ...
        cudaCheck(cudaMemcpyAsync(
          ...
          offset<dev_velo_track_hits_t>(manager),
          size<dev_velo_track_hits_t>(manager),
          cudaMemcpyDeviceToHost,
          cuda_stream));
      }
    }
