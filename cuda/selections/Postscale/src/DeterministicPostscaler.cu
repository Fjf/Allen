...
__device__ uint32_t postscaler::mix32( uint32_t state, uint32_t extra )
{
  return postscaler::mix( state + extra );
}
  ...
  const uint32_t gps_time_lo)
{
  ...
  auto x = postscaler::mix64( postscaler::mix32( postscaler::mix64( initial_value, gps_time_hi, gps_time_lo ), run_number ), evt_number_hi, evt_number_lo );
