...
namespace {
  ...
  struct MuonTable_t {
    ...
    MuonTable_t(std::vector<char>& data, const DeMuonDetector& det)
    {
      ...
      if (geom_version == 3) {
        // std::cout << "MuonTables: Thanks for your work trying to provide me a decoding, It's sad not to be understood
        // :(" << std::endl;
        ...
        for (auto& [sizeX, sizeY, offset, gridY] :
             {make_tuple(std::ref(padSizeX), std::ref(padSizeY), std::ref(padSizeOffset), std::ref(padGridY)),
              make_tuple(
                std::ref(stripXSizeX), std::ref(stripXSizeY), std::ref(stripXSizeOffset), std::ref(stripXGridY)),
              make_tuple(
          ...
          for (auto station : views::ints(0, nStations)) {
            ...
            for (auto region : views::ints(0, nRegions)) {
              ...
              for (auto [quarter, yx] : views::cartesian_product(views::ints(0, 4), yxRange)) {
                ...
                if (!pos) {
                  std::stringstream e;
                  e << t << " " << station << " " << region << " " << quarter << " " << gridX[gidx] << " "
                    << gridY[gidx] << " " << x << " " << y << "\n";
