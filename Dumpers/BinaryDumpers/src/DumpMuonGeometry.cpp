 ...
 *  @date   2022-06-10
 */
class DumpMuonGeometry final : public LHCb::Algorithm::MultiTransformer<
...
public:
  ...
  std::tuple<std::vector<char>, std::string> operator()(const DeMuonDetector& DeMuon) const override;
...
StatusCode DumpMuonGeometry::initialize() { return MultiTransformer::initialize(); }
  ...
  ) const
{
  ...
  if (geom_version == 3) {
    // std::cout << "MuonGeometry: Thanks for your work trying to provide me a decoding :), It's sad not to be
    // understood :(" << std::endl;
    ...
    std::array<std::array<unsigned int, Muon::Constants::maxTell40PCINumber>, Muon::Constants::maxTell40Number>
      tell40PCINumberOfActiveLink;
    std::array<
      std::array<std::array<unsigned int, Muon::Constants::maxNumberLinks>, Muon::Constants::maxTell40PCINumber>,
    ...
    std::array<
      std::array<
        std::array<unsigned int, Muon::Constants::maxNumberLinks * Muon::Constants::ODEFrameSize>,
        Muon::Constants::maxTell40PCINumber>,
      Muon::Constants::maxTell40Number>
      mapTileInTell40;
      ...
      for (int iTell = 0; iTell < cabling->getNumberOfTell40Board(); iTell++) {
        ...
        if (tell40->Tell40Number() > Muon::Constants::maxTell40Number)
          throw GaudiException(
            fmt::format(
              "Tell40 number {} greater than allowed {}", tell40->Tell40Number(), Muon::Constants::maxTell40Number),
            __FILE__,
            StatusCode::FAILURE);
          ...
          if (node > 0) {
            for (unsigned int ich = 0; ich < Muon::Constants::ODEFrameSize; ich++) {
              ...
              if (tileID.isValid()) {
                ...
                // std::cout << mapRegionOfLink[itell][ipci][ilink] << ", quarter: "<<
                // mapQuarterOfLink[itell][ipci][ilink] << std::endl;
              } // valid tileID
    ...
    output.write(whichStationIsTell40, tell40PCINumberOfActiveLink, mapRegionOfLink, mapQuarterOfLink, mapTileInTell40);
    ...
    //  	std::cout  << "            QuarterOfLink " << mapQuarterOfLink[i][j][k] << ", RegionOfLink " <<
    //  mapRegionOfLink[i][j][k] << std::endl;
    // 	for (auto l = 0; l < Muon::Constants::ODEFrameSize; l++){
    // 	  std::cout << "              ODE number " << l << std::endl;
    // 	  std::cout << "              TileInTell40 map " << mapTileInTell40[i][j][k * Muon::Constants::ODEFrameSize + l]
    // << std::endl;
    // 	}
    //      }
    //    }
