...
namespace host_global_event_cut {
  struct Parameters {
    ...
    HOST_INPUT(host_scifi_raw_bank_version_t, int) scifi_raw_bank_version;
    ...
    PROPERTY(count_scifi_t, "count_scifi", "count scifi clusters", bool) count_scifi;
    PROPERTY(count_ut_t, "count_ut", "count ut clusters", bool) count_ut;
    PROPERTY(min_scifi_clusters_t, "min_scifi_clusters", "minimum number SciFi of clusters", unsigned)
    min_scifi_clusters;
    PROPERTY(max_scifi_clusters_t, "max_scifi_clusters", "maximum number SciFi of clusters", unsigned)
    max_scifi_clusters;
    PROPERTY(min_ut_clusters_t, "min_ut_clusters", "minimum number UT of clusters", unsigned)
    min_ut_clusters;
    PROPERTY(max_ut_clusters_t, "max_ut_clusters", "maximum number UT of clusters", unsigned)
    max_ut_clusters;
    PROPERTY(min_total_clusters_t, "min_total_clusters", "minimum total number of clusters", unsigned)
    min_total_clusters;
    PROPERTY(max_total_clusters_t, "max_total_clusters", "maximum total number of clusters", unsigned)
    max_total_clusters;
  };
  ...
  void host_global_event_cut(host_global_event_cut::Parameters parameters)
  {
    ...
    auto const scifi_raw_bank_version = *parameters.scifi_raw_bank_version;
    ...
    for (unsigned event_index = 0; event_index < number_of_events; ++event_index) {
      ...
      if (parameters.count_scifi && scifi_raw_bank_version < 0) {
        warning_cout << "Requested to count SciFi clusters for the GEC, but no SciFi banks available\n";
      }
      else if (parameters.count_scifi && scifi_raw_bank_version >= 0) {
      ...
      if (parameters.count_ut && ut_raw_bank_version < 0) {
        warning_cout << "Requested to count UT clusters for the GEC, but no UT banks available\n";
      }
      else if (parameters.count_ut && ut_raw_bank_version >= 0) {
      ...
      if (
        n_UT_clusters <= parameters.max_ut_clusters &&
        n_UT_clusters >= parameters.min_ut_clusters &&
        n_SciFi_clusters <= parameters.max_scifi_clusters &&
        n_SciFi_clusters >= parameters.min_scifi_clusters &&
        num_combined_clusters <= parameters.max_total_clusters &&
        num_combined_clusters >= parameters.min_total_clusters) {
  ...
  private:
    Property<count_scifi_t> m_count_scifi {this, true};
    Property<count_ut_t> m_count_ut {this, true};
