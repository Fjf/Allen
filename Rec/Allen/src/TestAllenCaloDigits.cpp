...
class TestAllenCaloDigits final : public Gaudi::Functional::Consumer<void(HostBuffers const&,
                                                                          LHCb::Event::Calo::Digits const&,
                                                                          LHCb::Event::Calo::Digits const&)> {
...
public:
  ...
  /// Algorithm execution
  void operator()(const HostBuffers&,
                  LHCb::Event::Calo::Digits const&,
                  LHCb::Event::Calo::Digits const&) const override;

private:
  void compare(gsl::span<CaloDigit> const& allenDigits,
               LHCb::Event::Calo::Digits const& lhcbDigits, std::string const& calo) const;
};
...
void TestAllenCaloDigits::operator()(HostBuffers const& hostBuffers,
                                   ...
                                   LHCb::Event::Calo::Digits const& hcalDigits) const
{
  ...
  for (auto const& [allenDigits, lhcbDigits, calo] : {std::forward_as_tuple(hostBuffers.host_ecal_digits, ecalDigits, ecal),
                                                      std::forward_as_tuple(hostBuffers.host_hcal_digits, hcalDigits, hcal)}) {
    compare(allenDigits, lhcbDigits, calo);
  }
}

void TestAllenCaloDigits::compare(gsl::span<CaloDigit> const& allenDigits,
  ...
  for (auto d : lhcbDigits) {
    ...
    if (d.adc() != allenDigits[digit_index].adc) {
      ...
      error() << "LHCb digit at " << unsigned{idx} << " has different ADC: " << d.adc()
              << ", then Allen digit at " << digit_index << "with ADC: " << allenDigits[digit_index].adc << endmsg;
    }
  }
}
