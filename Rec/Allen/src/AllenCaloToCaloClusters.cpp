...
AllenCaloToCaloClusters::AllenCaloToCaloClusters(const std::string& name, ISvcLocator* pSvcLocator) :
  MultiTransformer(
    ...
    // Outputs
    {KeyValue {"AllenEcalClusters", "Allen/Calo/EcalCluster"},KeyValue {"AllenHcalClusters", "Allen/Calo/HcalCluster"} })
{}

std::tuple<LHCb::Event::Calo::Clusters, LHCb::Event::Calo::Clusters> AllenCaloToCaloClusters::operator()(const HostBuffers& host_buffers) const
{
  // avoid long names
  ...
  for (unsigned i = 0; i < number_of_ecal_clusters; i++) {
    ...
    if (msgLevel(MSG::DEBUG) && i<50 && i%5==0) {
      if(LHCb::Calo::isValid(seedCellID) ) debug() << "ECAL CellID " << seedCellID << " corresponding to dense ID " << cluster.center_id << " is invalid!"  << endmsg;
      debug() << " \t ECAL center_id = " << cluster.center_id << " cellID: " <<  seedCellID << ", e = " << cluster.e << ", x = " << cluster.x << ", y = " << cluster.y;
      for(unsigned j=0;j<Calo::Constants::max_neighbours;++j) debug() << " " << cluster.digits[j];
      debug() << endmsg; 
    }

    // Add the all digits, marking the seed ones
    EcalClusters.emplace_back( seedCellID,cluster.e,LHCb::CaloDigitStatus::Mask::SeedCell);
    for(unsigned j=0;j<Calo::Constants::max_neighbours;++j) {
        auto cellID = LHCb::Calo::DenseIndex::details::toCellID (cluster.digits[j]);
        EcalClusters.emplace_back(cellID,0.,LHCb::CaloDigitStatus::Mask::Undefined);
    }
  ...
  for (unsigned i = 0; i < number_of_hcal_clusters; i++) {
    ...
    if (msgLevel(MSG::DEBUG) && i<50 && i%5==0) {
      if(LHCb::Calo::isValid(seedCellID) ) debug() << "HCAL CellID " << seedCellID << " corresponding to dense ID " << cluster.center_id << " is invalid!"  << endmsg;
      debug() << " \t HCAL center_id = " << cluster.center_id << " cellID: " <<  seedCellID << ", e = " << cluster.e << ", x = " << cluster.x << ", y = " << cluster.y;
      for(unsigned j=0;j<Calo::Constants::max_neighbours;++j) debug() << " " << cluster.digits[j];
      debug() << endmsg;
    }
    ...
    for(unsigned j=0;j<Calo::Constants::max_neighbours;++j) {
        auto cellID = LHCb::Calo::DenseIndex::details::toCellID (cluster.digits[j]);
        HcalClusters.emplace_back(cellID,.0,LHCb::CaloDigitStatus::Mask::Undefined);
    }
    LHCb::Event::Calo::v2::Clusters::which_entries_t entries{iFirstEntry,Calo::Constants::max_neighbours +1};
    Gaudi::XYZPoint position{cluster.x, cluster.y,0.};
    ...
    if (msgLevel(MSG::DEBUG) ) {
        ...
        for (const auto &Cluster : EcalClusters) { 
            ...
            //const double et = e * sqrt( x * x + y * y ) / sqrt( x * x + y * y + z * z );
             ...
             if(i%5==0){
                 debug() << "Ecal cellID: " << cellID << " energy = " << e << ", x = " << x << ", y = " << y << ", z = " << z << endmsg;
                 auto digits = Cluster.entries();   
                 for (const auto & digit : digits ) debug() << "     cellID: " << digit.cellID() << " energy: " << digit.energy() << " Status: " << digit.status() << endmsg;
           }
           if (i>50) break;
           ++i;
        }
        ...
        for (const auto &Cluster : HcalClusters) {
            ...
            //const double et = e * sqrt( x * x + y * y ) / sqrt( x * x + y * y + z * z );
             ...
             if(i%5==0){
                 debug() << "Hcal cellID: " << cellID << " energy = " << e << ", x = " << x << ", y = " << y << ", z = " << z << endmsg;
                 auto digits = Cluster.entries();
                 for (const auto & digit : digits ) debug() << "     cellID: " << digit.cellID() << " energy: " << digit.energy() << " Status: " << digit.status() << endmsg;
           }
           if (i>50) break;
