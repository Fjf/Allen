...
public:
  ...
  zmq::message_t encode(const TObject& item) const {
   ...
   std::optional<size_t> send(zmq::socket_t& socket, const char* item, zmq::send_flags flags = zmq::send_flags::none) const {
  ...
  std::optional<size_t> send(zmq::socket_t& socket, zmq::message_t& msg, zmq::send_flags flags = zmq::send_flags::none) const {
