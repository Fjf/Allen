    ...
    class c_interop_string(c_char_p):
        ...
        def from_param(cls, param):
            ...
            raise TypeError("Cannot convert '{}' to '{}'".format(
                type(param).__name__, cls.__name__))
...
class Diagnostic(object):
    ...
    def children(self):
        class ChildDiagnosticsIterator(object):
            ...
            def __len__(self):
                return int(
                    conf.lib.clang_getNumDiagnosticsInSet(self.diag_set))
...
class Cursor(Structure):
    ...
    def enum_value(self):
        ...
        if not hasattr(self, '_enum_value'):
            ...
            if underlying_type.kind in (TypeKind.CHAR_U, TypeKind.UCHAR,
                                        TypeKind.CHAR16, TypeKind.CHAR32,
                                        TypeKind.USHORT, TypeKind.UINT,
...
class CompletionChunk(object):
    ...
    @CachedProperty
    def string(self):
        res = conf.lib.clang_getCompletionChunkCompletionString(
            self.cs, self.key)
...
class TranslationUnit(ClangObject):
    ...
    def from_source(cls,
        ...
        ptr = conf.lib.clang_parseTranslationUnit(
            index,
            fspath(filename) if filename is not None else None, args_array,
    ...
    def get_includes(self):
        ...
        def visitor(fobj, lptr, depth, includes):
            if depth > 0:
                loc = lptr.contents
                includes.append(
                    FileInclusion(loc.file, File(fobj), loc, depth))
    ...
    def save(self, filename):
        ...
        result = int(
            conf.lib.clang_saveTranslationUnit(self, fspath(filename),
                                               options))
        if result != 0:
...
class File(ClangObject):
    ...
    def from_name(translation_unit, file_name):
        ...
        return File(
            conf.lib.clang_getFile(translation_unit, fspath(file_name)))
...
functionList = [
    ...
    ("clang_CompilationDatabase_fromDirectory",
     [c_interop_string,
      POINTER(c_uint)], c_object_p, CompilationDatabase.from_result),
    ("clang_CompilationDatabase_getAllCompileCommands", [c_object_p],
     c_object_p, CompileCommands.from_result),
    ("clang_CompilationDatabase_getCompileCommands",
     [c_object_p, c_interop_string], c_object_p, CompileCommands.from_result),
    ...
    ("clang_codeCompleteGetDiagnostic", [CodeCompletionResults, c_int],
     Diagnostic),
    ...
    ("clang_getDiagnosticCategoryText", [Diagnostic], _CXString,
     _CXString.from_result),
    ("clang_getDiagnosticFixIt", [Diagnostic, c_uint,
                                  POINTER(SourceRange)], _CXString,
     _CXString.from_result),
    ...
    ("clang_parseTranslationUnit",
     [Index, c_interop_string, c_void_p, c_int, c_void_p, c_int,
      c_int], c_object_p),
    ("clang_reparseTranslationUnit", [TranslationUnit, c_int, c_void_p, c_int],
     c_int),
    ("clang_saveTranslationUnit", [TranslationUnit, c_interop_string, c_uint],
     c_int),
    ("clang_tokenize",
     [TranslationUnit, SourceRange,
      POINTER(POINTER(Token)),
      POINTER(c_uint)]),
    ("clang_visitChildren", [Cursor, callbacks['cursor_visit'], py_object],
     c_uint),
    ...
    ("clang_Cursor_getTemplateArgumentKind", [Cursor, c_uint],
    ...
    ("clang_Type_visitFields", [Type, callbacks['fields_visit'], py_object],
     c_uint),
]
