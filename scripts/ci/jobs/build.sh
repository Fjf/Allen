...
if [ "${TARGET}" = "HIP" ]; then
  ...
  cmake -DSTANDALONE=ON -GNinja -DTARGET_DEVICE=${TARGET} -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DSEQUENCES=${SEQUENCE} -DCPU_ARCH=haswell ${ADDITIONAL_OPTIONS} ${SOURCE_FOLDER}
  ...
  cmake -DSTANDALONE=ON -GNinja -DTARGET_DEVICE=${TARGET} -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
        -DSEQUENCES=${SEQUENCE} -DCPU_ARCH=haswell ${ADDITIONAL_OPTIONS} \
