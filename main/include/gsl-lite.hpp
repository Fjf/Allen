...
#define gsl_lite_VERSION \
  gsl_STRINGIFY(gsl_lite_MAJOR) "." gsl_STRINGIFY(gsl_lite_MINOR) "." gsl_STRINGIFY(gsl_lite_PATCH)
...
#if !defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS) && !defined(gsl_CONFIG_CONTRACT_VIOLATION_TERMINATES)
#define gsl_CONFIG_CONTRACT_VIOLATION_THROWS_V 0
#elif defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS) && !defined(gsl_CONFIG_CONTRACT_VIOLATION_TERMINATES)
...
#define gsl_STRINGIFY_(x) #x
  ...
  struct fail_fast : public std::logic_error {
    gsl_api explicit fail_fast(char const* const message) : std::logic_error(message) {}
  };
  ...
  namespace details {
    ...
    inline unsigned char to_uchar(unsigned x) gsl_noexcept { return static_cast<unsigned char>(x); }

#if gsl_HAVE(UNCAUGHT_EXCEPTIONS)

    inline unsigned char uncaught_exceptions() gsl_noexcept { return to_uchar(std::uncaught_exceptions()); }
  ...
  class final_action {
  public:
    gsl_api explicit final_action(F action) gsl_noexcept : action_(std::move(action)), invoke_(true) {}

    gsl_api final_action(final_action&& other) gsl_noexcept : action_(std::move(other.action_)), invoke_(other.invoke_)
    {
      other.invoke_ = false;
    }
  ...
  public:
    gsl_api explicit final_action_return(F&& action) gsl_noexcept : final_action<F>(std::move(action)),
    ...
    {
      if (details::uncaught_exceptions() != exception_count) this->dismiss();
    }
  ...
  public:
    gsl_api explicit final_action_error(F&& action) gsl_noexcept : final_action<F>(std::move(action)),
                                                                   exception_count(details::uncaught_exceptions())
    {}
  ...
  public:
    ...
    gsl_api final_action(final_action const& other) : action_(other.action_), invoke_(other.invoke_)
    {
      other.invoke_ = false;
    }
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
#define gsl_ENABLE_IF_INTEGRAL_T(T) , class = typename std::enable_if<std::is_integral<T>::value>::type
#else
#define gsl_ENABLE_IF_INTEGRAL_T(T)
  ...
  gsl_api inline gsl_constexpr unsigned char to_uchar(byte b) gsl_noexcept { return to_integer<unsigned char>(b); }

  gsl_api inline gsl_constexpr unsigned char to_uchar(int i) gsl_noexcept { return static_cast<unsigned char>(i); }
  ...
  gsl_api inline gsl_constexpr byte operator~(byte b) gsl_noexcept { return to_byte(~to_uchar(b)); }
  ...
  public:
    ...
    gsl_api gsl_constexpr14 span() gsl_noexcept : first_(gsl_nullptr), last_(gsl_nullptr) { Expects(size() == 0); }
    ...
    gsl_api gsl_constexpr14 span(pointer data_in, index_type size_in) : first_(data_in), last_(data_in + size_in)
    {
      Expects(size_in == 0 || (size_in > 0 && data_in != gsl_nullptr));
    }

    gsl_api gsl_constexpr14 span(pointer first_in, pointer last_in) : first_(first_in), last_(last_in)
    {
    ...
    template<class U>
    gsl_api gsl_constexpr14 span(U*& data_in, index_type size_in) : first_(data_in), last_(data_in + size_in)
    {
      Expects(size_in == 0 || (size_in > 0 && data_in != gsl_nullptr));
    }

    template<class U>
    gsl_api gsl_constexpr14 span(U* const& data_in, index_type size_in) : first_(data_in), last_(data_in + size_in)
    {
      Expects(size_in == 0 || (size_in > 0 && data_in != gsl_nullptr));
    }
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
      ,
      class = typename std::enable_if<std::is_convertible<value_type (*)[], element_type (*)[]>::value>::type
#endif
      >
    gsl_api gsl_constexpr span(element_type (&arr)[N]) gsl_noexcept : first_(gsl_ADDRESSOF(arr[0])),
                                                                      last_(gsl_ADDRESSOF(arr[0]) + N)
    {}
#endif // deprecate
    ...
    template<class U, size_t N>
    gsl_api gsl_constexpr span(std::array<U, N>& arr) : first_(arr.data()), last_(arr.data() + N)
    {}

    template<class U, size_t N>
    gsl_api gsl_constexpr span(std::array<U, N> const& arr) : first_(arr.data()), last_(arr.data() + N)
    {}
...
#endif
      >
    gsl_api gsl_constexpr span(std::array<value_type, N>& arr) : first_(arr.data()), last_(arr.data() + N)
    {}
...
#endif
      >
    gsl_api gsl_constexpr span(std::array<value_type, N> const& arr) : first_(arr.data()), last_(arr.data() + N)
    {}
    ...
    template<
      class Container,
      class = typename std::enable_if<details::can_construct_span_from<Container, element_type>::value>::type>
    gsl_api gsl_constexpr span(Container& cont) : first_(cont.data()), last_(cont.data() + cont.size())
    {}

    template<
      class Container,
      class = typename std::enable_if<
        std::is_const<element_type>::value && details::can_construct_span_from<Container, element_type>::value>::type>
    gsl_api gsl_constexpr span(Container const& cont) : first_(cont.data()), last_(cont.data() + cont.size())
    {}
    ...
    gsl_api gsl_constexpr span(shared_ptr<element_type> const& ptr) :
      first_(ptr.get()), last_(ptr.get() ? ptr.get() + 1 : 0)
    {}
#endif
...
#else
    gsl_constexpr span(span const& other) : first_(other.begin()), last_(other.end()) {}
#endif
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
      ,
      class = typename std::enable_if<std::is_convertible<U (*)[], element_type (*)[]>::value>::type
#endif
      >
    gsl_api gsl_constexpr span(span<U> const& other) : first_(other.begin()), last_(other.end())
    {}
    ...
    {
      Expects(offset <= this->size() && count + offset <= this->size());
      return span(this->data() + offset, count);
    }

    // 26.7.3.4 Observers [span.obs]

    gsl_api gsl_constexpr index_type size() const gsl_noexcept { return narrow_cast<index_type>(last_ - first_); }
    ...
    gsl_api gsl_constexpr bool empty() const gsl_noexcept { return size() == 0; }

    // 26.7.3.5 Element access [span.elem]

    gsl_api gsl_constexpr reference operator[](index_type index) const { return at(index); }
    ...
    gsl_api gsl_constexpr iterator begin() const gsl_noexcept { return iterator(first_); }

    gsl_api gsl_constexpr iterator end() const gsl_noexcept { return iterator(last_); }
    ...
    gsl_api gsl_constexpr reverse_iterator rbegin() const gsl_noexcept { return reverse_iterator(end()); }

    gsl_api gsl_constexpr reverse_iterator rend() const gsl_noexcept { return reverse_iterator(begin()); }

    gsl_api gsl_constexpr const_reverse_iterator crbegin() const gsl_noexcept { return const_reverse_iterator(cend()); }

    gsl_api gsl_constexpr const_reverse_iterator crend() const gsl_noexcept { return const_reverse_iterator(cbegin()); }
    ...
    gsl_api gsl_constexpr index_type length() const gsl_noexcept { return size(); }

    // member length_bytes() deprecated since 0.29.0

    gsl_api gsl_constexpr index_type length_bytes() const gsl_noexcept { return size_bytes(); }
#endif
  ...
  gsl_api inline void copy(span<T> src, span<U> dest)
  {
    ...
    static_assert(
      std::is_assignable<U&, T const&>::value, "Cannot assign elements of source span to elements of destination span");
#endif
  ...
  template<class T>
  gsl_api inline gsl_constexpr span<T> make_span(T * ptr, typename span<T>::index_type count)
  {
  ...
  template<class T, size_t N>
  gsl_api inline gsl_constexpr span<const T> make_span(std::array<T, N> const& arr)
  {
  ...
  gsl_api inline gsl_constexpr auto make_span(Container & cont)->span<typename Container::value_type>
  {
    return span<typename Container::value_type>(cont);
  }

  template<class Container, class = decltype(std::declval<Container>().data())>
  gsl_api inline gsl_constexpr auto make_span(Container const& cont)->span<const typename Container::value_type>
  {
  ...
  template<class Container>
  gsl_api inline gsl_constexpr span<typename Container::value_type> make_span(with_container_t, Container & cont)
    gsl_noexcept
  {
  ...
  template<class T>
  gsl_api inline gsl_constexpr span<const byte> byte_span(T const& t) gsl_noexcept
  {
  ...
  public:
...
#if gsl_HAVE(NULLPTR)
    gsl_api gsl_constexpr basic_string_span(std::nullptr_t ptr) gsl_noexcept : span_(ptr, index_type(0)) {}
#endif

    gsl_api gsl_constexpr basic_string_span(pointer ptr) : span_(remove_z(ptr, std::numeric_limits<index_type>::max()))
    {}

    gsl_api gsl_constexpr basic_string_span(pointer ptr, index_type count) : span_(ptr, count) {}

    gsl_api gsl_constexpr basic_string_span(pointer firstElem, pointer lastElem) : span_(firstElem, lastElem) {}

    template<std::size_t N>
    gsl_api gsl_constexpr basic_string_span(element_type (&arr)[N]) : span_(remove_z(gsl_ADDRESSOF(arr[0]), N))
    {}
    ...
    basic_string_span(std::array<typename details::remove_const<element_type>::type, N> const& arr) :
      span_(remove_z(arr))
    {}
    ...
    template<
      class Container,
      class = typename std::enable_if<
        !details::is_std_array<Container>::value && !details::is_basic_string_span<Container>::value &&
    ...
    template<
      ...
      class = typename std::enable_if<
        ...
        type>
    gsl_api gsl_constexpr basic_string_span(Container const& cont) : span_((cont))
    {}
    ...
    template<class Container>
    gsl_api gsl_constexpr basic_string_span(with_container_t, Container& cont) : span_(with_container, cont)
    {}
#endif
    ...
    template<
      class U,
      class =
        typename std::enable_if<std::is_convertible<typename basic_string_span<U>::pointer, pointer>::value>::type>
    ...
    gsl_api gsl_constexpr basic_string_span first(index_type count) const { return span_.first(count); }

    gsl_api gsl_constexpr basic_string_span last(index_type count) const { return span_.last(count); }

    gsl_api gsl_constexpr basic_string_span subspan(index_type offset) const { return span_.subspan(offset); }
    ...
    gsl_api gsl_constexpr index_type length() const gsl_noexcept { return span_.size(); }

    gsl_api gsl_constexpr index_type size() const gsl_noexcept { return span_.size(); }

    gsl_api gsl_constexpr index_type length_bytes() const gsl_noexcept { return span_.size_bytes(); }

    gsl_api gsl_constexpr index_type size_bytes() const gsl_noexcept { return span_.size_bytes(); }

    gsl_api gsl_constexpr bool empty() const gsl_noexcept { return size() == 0; }

    gsl_api gsl_constexpr reference operator[](index_type idx) const { return span_[idx]; }

    gsl_api gsl_constexpr reference operator()(index_type idx) const { return span_[idx]; }

    gsl_api gsl_constexpr pointer data() const gsl_noexcept { return span_.data(); }

    gsl_api iterator begin() const gsl_noexcept { return span_.begin(); }

    gsl_api iterator end() const gsl_noexcept { return span_.end(); }

    gsl_api reverse_iterator rbegin() const gsl_noexcept { return span_.rbegin(); }

    gsl_api reverse_iterator rend() const gsl_noexcept { return span_.rend(); }

    // const version not in p0123r2:

    gsl_api const_iterator cbegin() const gsl_noexcept { return span_.cbegin(); }

    gsl_api const_iterator cend() const gsl_noexcept { return span_.cend(); }

    gsl_api const_reverse_iterator crbegin() const gsl_noexcept { return span_.crbegin(); }

    gsl_api const_reverse_iterator crend() const gsl_noexcept { return span_.crend(); }
  ...
  private:
...
#if gsl_HAVE(ARRAY)
    template<size_t N>
    gsl_api static gsl_constexpr14 span_type
    remove_z(std::array<typename details::remove_const<element_type>::type, N>& arr)
    {
    ...
    template<size_t N>
    gsl_api static gsl_constexpr14 span_type
    remove_z(std::array<typename details::remove_const<element_type>::type, N> const& arr)
    {
      return remove_z(gsl_ADDRESSOF(arr[0]), narrow_cast<std::size_t>(N));
    }
#endif
  ...
  template<class T, class U, class = typename std::enable_if<!details::is_basic_string_span<U>::value>::type>
  ...
  template<class T>
  gsl_api inline gsl_constexpr14 bool operator==(basic_string_span<T> const& l, basic_string_span<T> const& r)
    gsl_noexcept
  {
    return l.size() == r.size() && std::equal(l.begin(), l.end(), r.begin());
  }
...
gsl_api inline std::basic_string< typename std::remove_const<T>::type > to_string( basic_string_span<T> spn )
{
  ...
  gsl_api inline std::string to_string(string_span const& spn) { return std::string(spn.data(), spn.length()); }

  gsl_api inline std::string to_string(cstring_span const& spn) { return std::string(spn.data(), spn.length()); }

#if gsl_HAVE(WCHAR)

  gsl_api inline std::wstring to_string(wstring_span const& spn) { return std::wstring(spn.data(), spn.length()); }

  gsl_api inline std::wstring to_string(cwstring_span const& spn) { return std::wstring(spn.data(), spn.length()); }
  ...
  template<typename Traits>
  gsl_api std::basic_ostream<wchar_t, Traits>& operator<<(
  ...
  template<class Container>
  gsl_api inline span<typename std::remove_pointer<typename Container::pointer>::type> ensure_z(Container & cont)
  {
...
namespace std {
  ...
  public:
    std::size_t operator()(gsl::byte v) const gsl_noexcept { return gsl::to_integer<std::size_t>(v); }
  };
