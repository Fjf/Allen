...
def test_add_event_list_combiners():
    ...
    order, score = get_execution_list_for(root)
    should_be_algs = add_event_list_combiners(order)
    algs = ((initialize_event_lists(),
             None), (pre0, None), (dec0, pre0), (NOT_pre0, None),
            (NOT_dec0, None), (NOT_pre0_OR_NOT_dec0,
                               None), (pre1,
                                       parse_boolean("(~PRE0_st0 | ~X_st0)")),
            (pre1_AND_NOT_pre0_OR_NOT_dec0,
             None), (dec1, parse_boolean("(PRE1_st0 & (~PRE0_st0 | ~X_st0))")))
    assert algs == should_be_algs
