  ...
  def generate(cls,
    ...
    for _, algorithm in iter(algorithms.items()):
      for parameter_name, _ in iter(algorithm.__slots__.items()):
        parameter_t = Sequence.clean_prefix(getattr(algorithm, parameter_name))
        algorithm_name = algorithm.getType()
        algorithm_namespace = algorithm.namespace()
        if parameter_t in parameters:
          parameters[parameter_t].append((algorithm_name, algorithm_namespace, parameter_name))
        else:
          parameters[parameter_t] = [(algorithm_name, algorithm_namespace, parameter_name)]
    ...
    for algorithm_name, algorithm in iter(algorithms.items()):
      ...
      s += "  " + algorithm.namespace(
      ) + "::" + algorithm.getType() + "<std::tuple<"
      i = 0
      # Add parameters
      for parameter_name, _ in iter(algorithm.__slots__.items()):
        parameter_t = Sequence.clean_prefix(getattr(algorithm, parameter_name))
