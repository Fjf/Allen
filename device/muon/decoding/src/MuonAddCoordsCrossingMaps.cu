  ...
  const Allen::Context& context) const
{
  ...
  if (muon_bank_version == 2)
    global_function(muon_add_coords_crossing_maps_tell1)(
      dim3(size<dev_event_list_t>(arguments)), property<block_dim_t>(), context)(arguments);
  else if (muon_bank_version == 3)
    global_function(muon_add_coords_crossing_maps_tell40)(
      dim3(size<dev_event_list_t>(arguments)), property<block_dim_t>(), context)(arguments);
  else
    throw StrException("MuonAddCoordsCrossingMaps : unrecognized muon raw bank version \n");
}

__global__ void muon_add_coords_crossing_maps::muon_add_coords_crossing_maps_tell1(
...
__global__ void muon_add_coords_crossing_maps::muon_add_coords_crossing_maps_tell40(
  muon_add_coords_crossing_maps::Parameters parameters)
{
  ...
  for (unsigned i = 0; i < Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters;
    ...
    const bool pad = (station == 0 && region > 1) || (station == 2 && region == 0) || (station == 3 && region == 0) ||
                     (station == 3 && region == 3);
    ...
    if (pad) {
      ...
      const uint64_t compact_hit = (((uint64_t)(start_index & 0x7FFF)) << 48) |
                                   (((uint64_t)(start_index & 0xFFFF)) << 32) | ((layout1.xGrid() & 0x3FFF) << 18) |
                                   ((layout2.yGrid() & 0x3FFF) << 4) |
                                   (((tile.id() & Muon::MuonBase::MaskStation) >> Muon::MuonBase::ShiftStation) & 0xF);
      ...
      for (unsigned digitsOneIndex = start_index + threadIdx.x; digitsOneIndex < mid_index;
        ...
        for (unsigned digitsTwoIndex = mid_index; digitsTwoIndex < end_index; digitsTwoIndex++) {
          ...
          if (keyX == candidateX && keyY == candidateY) {
            ...
            // printf(" ADDING A CROSSED PAD WITH TILES %u %u \n",  storage_tile_id[digitsOneIndex],
            // storage_tile_id[digitsTwoIndex]);
