  ...
  const Allen::Context& context) const
{
  ...
  if (muon_bank_version == 2) 
    global_function(muon_add_coords_crossing_maps_tell1)(dim3(size<dev_event_list_t>(arguments)), property<block_dim_t>(), context)(arguments);
  else if (muon_bank_version == 3) 
    global_function(muon_add_coords_crossing_maps_tell40)(dim3(size<dev_event_list_t>(arguments)), property<block_dim_t>(), context)(arguments);
  else
    throw StrException("MuonAddCoordsCrossingMaps : unrecognized muon raw bank version \n");

  print<dev_muon_tile_used_t>(arguments);
  print<dev_atomics_index_insert_t>(arguments);
  print<dev_station_ocurrences_sizes_t>(arguments);
  print<dev_muon_compact_hit_t>(arguments);

}

__global__ void muon_add_coords_crossing_maps::muon_add_coords_crossing_maps_tell1(muon_add_coords_crossing_maps::Parameters parameters) {
  ...
  for (unsigned i = 0; i < Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters; i++) {
...
__global__ void muon_add_coords_crossing_maps::muon_add_coords_crossing_maps_tell40(muon_add_coords_crossing_maps::Parameters parameters){

  printf("Entering muon_add_coords_crossing_maps_tell40 \n");
  const unsigned event_number = parameters.dev_event_list[blockIdx.x];
  ...
  for (unsigned i = 0; i < Muon::Constants::n_stations * Muon::Constants::n_regions * Muon::Constants::n_quarters; i++) {
    ...
    printf("i %u, start index %u, mid_index %u, end index %u \n", i, start_index, mid_index, end_index);
    if (start_index == end_index) continue;
    const auto tile = Muon::MuonTileID(storage_tile_id[start_index]);
    const auto station = tile.station();
    const auto region = tile.region();

    const bool pad = (station == 0 && region > 1 ) || (station == 2 && region == 0) || (station == 3 && region == 0) || (station == 3 && region == 3);
	...
	(((tile.id() & Muon::MuonBase::MaskStation) >> Muon::MuonBase::ShiftStation) & 0xF);
	  ...
	  printf(" ADDING A CROSSED PAD WITH TILES %u %u \n",  storage_tile_id[digitsOneIndex], storage_tile_id[digitsTwoIndex]);
	...
	const auto hit_layout = Muon::MuonTileID::layout(storage_tile_id[index]);
        int condition = (hit_layout == layout1 ? 1 : 2);
    ...
    printf("End of muon_add_coords_crossing_maps_tell40 \n");
  } 
}
