  ...
  const unsigned number_of_events)
{
  ...
  for (unsigned idx = threadIdx.x; idx < number_of_events; idx += blockDim.x) {
    ...
    const unsigned bxt = static_cast<unsigned int>(LHCb::ODIN{parameters.dev_odin_data[event_number]}.bunchCrossingType());
  ...
  const Allen::Context& context) const
{
  ...
  global_function(odin_beamcrossingtype_kernel)(
