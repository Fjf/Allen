  ...
  std::tuple<const VertexFit::TrackMVAVertex&> input) const
{
  ...
  const bool decision =
    ...
    (vertex.x * vertex.x + vertex.y * vertex.y) > parameters.DMSoftMinRho2 && (vertex.z > parameters.DMSoftMinZ) &&
    (vertex.z < parameters.DMSoftMaxZ) && vertex.doca < parameters.DMSoftMaxDOCA &&
