  ...
  std::tuple<const VertexFit::TrackMVAVertex&> input) const
{
  ...
  const bool decision = vertex.chi2 > 0 && vertex.mdimu > parameters.minMass &&
    vertex.minpt > parameters.minTrackPt && vertex.p1 > parameters.minTrackP && vertex.p2 > parameters.minTrackP &&
