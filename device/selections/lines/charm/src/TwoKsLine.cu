  ...
  std::tuple<const VertexFit::TrackMVAVertex&, const unsigned, const unsigned> input)
{
  ...
  for (unsigned i = vertex1_id + 1; i < n_svs; i++) {
    ...
    const bool decision =
      ...
      vertex1.m(Allen::mPi, Allen::mPi) > parameters.minM_Ks &&
      vertex1.m(Allen::mPi, Allen::mPi) < parameters.maxM_Ks && vertex1.pt() > parameters.minComboPt_Ks &&
      vertex1.cos > parameters.minCosOpening && vertex1.dira > parameters.minCosDira &&
      vertex1.p1 > parameters.minTrackP_piKs && vertex1.p2 > parameters.minTrackP_piKs &&
      vertex1.ip1 * vertex1.ip2 / vertex1.vertex_ip > parameters.min_combip &&
