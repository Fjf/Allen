  ...
  std::tuple<const VertexFit::TrackMVAVertex&, const unsigned, const unsigned> input)
{
  ...
  for (unsigned i = threadIdx.y + vertex1_id + 1; i < n_svs; i += blockDim.y) {
    ...
    const bool decision =
      ...
      vertex1.m(139.57061, 139.57061) > parameters.minM_Ks && vertex1.m(139.57061, 139.57061) < parameters.maxM_Ks &&
      ...
      vertex2.minipchi2 > parameters.minTrackIPChi2_Ks && vertex2.m(139.57061, 139.57061) > parameters.minM_Ks &&
      vertex2.m(139.57061, 139.57061) < parameters.maxM_Ks && vertex2.pt() > parameters.minComboPt_Ks &&
