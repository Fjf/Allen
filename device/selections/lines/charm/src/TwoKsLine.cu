  ...
  std::tuple<const VertexFit::TrackMVAVertex&, const unsigned, const unsigned> input)
{
  ...
  for (unsigned i = threadIdx.y + vertex1_id + 1; i < n_svs; i += blockDim.y) {
    ...
    const bool decision =
        vertex1.chi2 < parameters.maxVertexChi2 &&
        vertex1.eta > parameters.minEta_Ks && vertex1.eta < parameters.maxEta_Ks &&
        vertex1.minipchi2 > parameters.minTrackIPChi2_Ks &&
        vertex1.m(139.57061, 139.57061) > parameters.minM_Ks &&
        vertex1.m(139.57061, 139.57061) < parameters.maxM_Ks &&
        vertex1.pt() > parameters.minComboPt_Ks &&
        vertex1.cos > parameters.minCosOpening &&
        vertex1.dira > parameters.minCosDira &&
        vertex1.p1 > parameters.minTrackP_piKs && vertex1.p2 > parameters.minTrackP_piKs &&
        vertex1.ip1 * vertex1.ip2 / vertex1.vertex_ip > parameters.min_combip && vertex1.minpt > parameters.minTrackPt_piKs &&
        vertex2.chi2 < parameters.maxVertexChi2 &&
        vertex2.eta > parameters.minEta_Ks && vertex2.eta < parameters.maxEta_Ks &&
        vertex2.minipchi2 > parameters.minTrackIPChi2_Ks &&
        vertex2.m(139.57061, 139.57061) > parameters.minM_Ks &&
        vertex2.m(139.57061, 139.57061) < parameters.maxM_Ks &&
        vertex2.pt() > parameters.minComboPt_Ks &&
        vertex2.cos > parameters.minCosOpening &&
        vertex2.dira > parameters.minCosDira &&
        vertex2.p1 > parameters.minTrackP_piKs && vertex2.p2 > parameters.minTrackP_piKs &&
        vertex2.ip1 * vertex2.ip2 / vertex2.vertex_ip > parameters.min_combip && vertex2.minpt > parameters.minTrackPt_piKs;
