  ...
  std::tuple<const Allen::Views::Physics::CompositeParticle> input)
{
  ...
  bool decision = vtx.vertex().z() < parameters.maxZ && vtx.is_dimuon() && vtx.doca12() < parameters.maxDoca && vtx.mdimu() >= parameters.minMass &&
				     vtx.minpt() >= parameters.minTrackPt && vtx.minp() >= parameters.minTrackP &&
    vtx.vertex().chi2() < parameters.maxVertexChi2 && vtx.vertex().z() >= parameters.minZ && vtx.charge() == parameters.CombCharge;
