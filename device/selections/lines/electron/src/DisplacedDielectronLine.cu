...
displaced_dielectron_line::displaced_dielectron_line_t::get_input(
  ...
  const unsigned i)
{
  const auto event_vertices = static_cast<const Allen::Views::Physics::CompositeParticles&>(parameters.dev_particle_container[0].particle_container(event_number));
