...
{
  set_size<dev_routingbits_t>(arguments, (first<host_number_of_events_t>(arguments)));
  set_size<host_routingbits_t>(arguments, (first<host_number_of_events_t>(arguments)));
}
  ...
  const Allen::Context& context) const
{
  ...
  global_function(routingbits_writer)(
  ...
  const RoutingBitsConfiguration::RoutingBits* dev_routingbits_conf)
{
  ...
  for (unsigned event_index = blockIdx.x * blockDim.x + threadIdx.x; event_index < number_of_events;
    ...
    for (unsigned bit_index = 0; bit_index < sizeof(dev_routingbits_conf->bits) / sizeof(dev_routingbits_conf->bits[0]);
         bit_index++) {
