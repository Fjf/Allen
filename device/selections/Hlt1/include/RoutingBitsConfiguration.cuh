...
namespace RoutingBitsConfiguration {
  ...
  struct RoutingBits {
    ...
    int bits[6] {33, 34, 35, 36, 47, 48};

    // expression used to evaluate routing bits - test values implemented so far
    // Run 2 implementation can be found here:
    // https://gitlab.cern.ch/lhcb/Hlt/blob/2018-patches/Hlt%2FHltConf%2Fpython%2FHltConf%2FHltOutput.py#L374
  ...
  struct AssociatedLines {
    int routing_bit;
    unsigned n_lines;
    int line_numbers[20]; // indices of HLT1 lines used to set each routign bits with an OR logic. For now 20 is a fixed
                          // number - will be replaced with the maximum number of lines used in the logic once this is
                          // known
  };
