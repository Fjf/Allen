...
__global__ void scifi_raw_bank_decoder_v4_kernel(
...
__device__ void direct_decode_raw_bank_v4(
  ...
  SciFi::SciFiRawBank const& raw_bank,
  ...
  SciFi::Hits& hits)
{
  ...
  if (last > it) {
    ...
    for (unsigned i_cluster = threadIdx.y; i_cluster < number_of_clusters; i_cluster += blockDim.y) {
      ...
      uint8_t cluster_fraction = SciFi::fraction(c);
      uint32_t ch = geom.bank_first_channel[raw_bank.sourceID] + SciFi::channelInBank(c);
      ...
      const uint8_t pseudoSize = SciFi::cSize(c) ? 0 : 4;
 ...
 */
template<bool mep_layout>
__global__ void scifi_direct_decoder_v4_kernel(
  scifi_raw_bank_decoder_v4::Parameters parameters,
  const char* scifi_geometry)
{
  const unsigned event_number = parameters.dev_event_list[blockIdx.x];
  ...
  for (unsigned i = threadIdx.x; i < SciFi::Constants::n_consecutive_raw_banks; i += blockDim.x) {
    ...
    const auto raw_bank = scifi_raw_event.raw_bank(current_raw_bank);
...
{
  global_function(
    runtime_options.mep_layout ? scifi_raw_bank_decoder_v4_kernel<true> : scifi_raw_bank_decoder_v4_kernel<false>)(
    dim3(size<dev_event_list_t>(arguments)), property<raw_bank_decoder_block_dim_t>(), context)(
    arguments, constants.dev_scifi_geometry);

  global_function(
    runtime_options.mep_layout ? scifi_direct_decoder_v4_kernel<true> : scifi_direct_decoder_v4_kernel<false>)(
