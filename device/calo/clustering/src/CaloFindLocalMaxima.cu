...
__device__ void local_maxima(CaloDigit* digits,
  ...
  const char* raw_hcal_geometry)
{
  ...
  for (auto event_number = blockIdx.x * blockDim.x; event_number < number_of_events;
    ...
    // ECal
    local_maxima(parameters.dev_ecal_digits + (event_number * ecal_geometry.max_cellid),
                 parameters.dev_ecal_num_clusters + event_number,
                 ecal_geometry);

    // HCal
    local_maxima(parameters.dev_hcal_digits + (event_number * hcal_geometry.max_cellid),
  ...
  cudaEvent_t&) const
{
  ...
  // Find local maxima.
  global_function(calo_find_local_maxima)(grid_size, dim3(property<block_dim_x_t>().get()), cuda_stream)(
