...
__device__ void decode(const char* data, const uint32_t* offsets,
                       const uint32_t* sizes, const uint32_t* types,
                       unsigned const event_number, CaloDigit* digits,
                       unsigned const number_of_digits,
                       CaloGeometry const& geometry) {
  auto raw_event = RawEvent{ data, offsets, sizes, types, event_number };

  for (unsigned bank_number = threadIdx.x;
       bank_number < raw_event.number_of_raw_banks; bank_number += blockDim.x) {
    auto raw_bank = raw_event.raw_bank(bank_number);

    if
      constexpr(decoding_version < 4) { // old decoding
        ...
        while (raw_bank.data < raw_bank.end) {
          ...
          for (unsigned int bit_num = 0; 32 > bit_num; ++bit_num) {
            ...
            uint16_t index =
                geometry.channels
                    [(code - geometry.code_offset) * geometry.card_channels +
                     bit_num];
      ...
      if (!((source_id >> 11) == 11))
        continue; // Only decode Ecal banks

      auto raw_event_fiberCheck =
          RawEvent{ data, offsets, sizes, types, event_number };
      auto raw_bank_fiberCheck = raw_event_fiberCheck.raw_bank(bank_number);

      auto get_data = [](uint32_t const* raw_data) {
        auto d = *raw_data;
        if
          constexpr(decoding_version == 4) { // big endian
            d = ((d >> 24) & 0x000000FF) | ((d >> 8) & 0x0000FF00) |
      ...
      for (int ifeb = 0; ifeb < 3; ifeb++) {
        ...
        if (code == 0)
          continue; // No FEB linked to the TELL40 slot

        // ... and readout data
        for (unsigned int bitNum = 0; 32 > bitNum; bitNum++) {
          //	  if (logF) printf("bitNum = %u\n",bitNum);
          if (nADC % 8 ==
              0) { // Check fibers pattern, 1 fiber corresponds to 8 ADC (96b)
            ...
            if (pattern1 == fibMask1 && pattern2 == fibMask2 &&
                pattern3 == fibMask3)
              isFiberOff = true;
            else
              isFiberOff = false;
          ...
          uint16_t index =
              geometry.channels[(index_code) * geometry.card_channels + bitNum];

          // Ignore cells with invalid indices; these include LED diodes.
          if (index < number_of_digits && !isFiberOff) {
            // if (adc > 500 && adc < 100000) {
            //   printf(
            //     ">500    index = %u, ADC = %d , sourceID = %u, ife = %u,
            // code = %u,  decoding version = %u, adc = "
            //     "%03x\n",
            //     index,
            //     adc,
            //     source_id,
            ...
            //   printf(
            //     "index = %u, code = %u, bitNum = %u, ADC = %d, ADChex = %03x,
            // sourceID = %u, ife = %u,  decoding "
            //     "version = %u\n",
            //     index,
            //     code,
            //     bitNum,
            ...
            //     ifeb,
            //     decoding_version);
            // if (logF) printf("index = %u, code = %u, bitNum = %u, ADC = %d,
            // ADChex = %03x, sourceID = %u, ife = %u,
            // decoding version = %u\n", index, code, bitNum, adc,
            // adc+256,source_id, ifeb, decoding_version);
            // if (log && bitNum == 0)
            //   printf(
            //     " |  SourceID : %u |  FeBoard : %u |  Channel : %u |  ADC
            // value = %d %d\n",
            //     source_id,
            //     code,
            //     bitNum,
            ...
            // } else {
            //	if (logF) printf("index = %u, number_of_digits =
...
__global__ void calo_decode_dispatch(calo_decode::Parameters parameters,
                                     const char* raw_ecal_geometry) {
  unsigned const event_number = parameters.dev_event_list[blockIdx.x];

  // ECal
  auto ecal_geometry = CaloGeometry(raw_ecal_geometry);
  auto const ecal_digits_offset =
      parameters.dev_ecal_digits_offsets[event_number];

  decode<Calo::RawEvent<mep_layout>, decoding_version>(
      parameters.dev_ecal_raw_input, parameters.dev_ecal_raw_input_offsets,
      parameters.dev_ecal_raw_input_sizes, parameters.dev_ecal_raw_input_types,
      event_number, &parameters.dev_ecal_digits[ecal_digits_offset],
      parameters.dev_ecal_digits_offsets[event_number + 1] - ecal_digits_offset,
      ecal_geometry);
}

void calo_decode::calo_decode_t::set_arguments_size(
    ArgumentReferences<Parameters> arguments, const RuntimeOptions&,
    const Constants&, const HostBuffers&) const {
  set_size<dev_ecal_digits_t>(arguments,
                              first<host_ecal_number_of_digits_t>(arguments));
}

void calo_decode::calo_decode_t::operator()(
    const ArgumentReferences<Parameters>& arguments,
    const RuntimeOptions& runtime_options, const Constants& constants,
    HostBuffers& host_buffers, const Allen::Context& context) const {
  initialize<dev_ecal_digits_t>(arguments, 0x7F, context);
  auto const bank_version = first<host_raw_bank_version_t>(arguments);

  // Ensure the bank version is supported
  if (bank_version > 4) {
    throw StrException("Calo bank version not supported (" +
                       std::to_string(bank_version) + ")");
  }

  auto fn = runtime_options.mep_layout
                ? (bank_version == 4 ? calo_decode_dispatch<true, 4>
                                     : calo_decode_dispatch<true, 3>)
                : (bank_version == 4 ? calo_decode_dispatch<false, 4>
                                     : calo_decode_dispatch<false, 3>);

  global_function(fn)(dim3(size<dev_event_list_t>(arguments)),
                      dim3(property<block_dim_x_t>().get()),
                      context)(arguments, constants.dev_ecal_geometry);

  if (runtime_options.fill_extra_host_buffers) {
    safe_assign_to_host_buffer<dev_ecal_digits_offsets_t>(
        host_buffers.host_ecal_digits_offsets, arguments, context);
    safe_assign_to_host_buffer<dev_ecal_digits_t>(host_buffers.host_ecal_digits,
                                                  arguments, context);
  }
}
