...
namespace Allen {
  namespace Views {
    namespace Physics {
      ...
      // TODO: Should PVs and SVs share a view?
      struct PrimaryVertex {
        constexpr static unsigned nb_elements_vrt = 3;
        // 6 covariance matrix elements + chi2 + ndof
        constexpr static unsigned nb_elements_cov = 8;

      private:
        const float* m_base_pointer = nullptr;
        const unsigned m_index = 0;
        const unsigned m_total_number_of_vrts = 0;

      public:
        __host__ __device__
        PrimaryVertex(const char* base_pointer, const unsigned index, const unsigned total_number_of_vrts) :
          m_base_pointer(reinterpret_cast<const float*>(base_pointer)),
          m_index(index),
      ...
      public:
        __host__ __device__ PrimaryVertices(
          const char* base_pointer,
        ...
        __host__ __device__ const PrimaryVertex vertex(const unsigned sv_index) const
        {
          assert(sv_index < m_size);
          return PrimaryVertex {m_base_pointer, m_offset + sv_index, m_total_number_of_vrts};
        }
      };
