...
struct HltSelRepRBHits {
  ...
  __device__ unsigned numberOfSeq() const { return (unsigned) (m_location[0] & 0xFFFFu); }

  __device__ unsigned hitsLocation() const { return (numberOfSeq() / 2 + 1); }
  ...
  __device__ unsigned seqSize(unsigned iSeq) const { return (seqEnd(iSeq) - seqBegin(iSeq)); }
...
struct HltSelRepRBStdInfo {
  ...
  __device__ __host__ unsigned sizeStored() const { return (m_location[0] >> 16) & 0xFFFFu; }

  __device__ __host__ static unsigned sizeStoredFromPtr(const uint32_t* ptr) { return (ptr[0] >> 16) & 0xFFFFu; }

  __device__ __host__ bool writeStdInfo() const { return sizeStored() >= Hlt1::maxStdInfoEvent; }

  __device__ __host__ unsigned numberOfObj() const { return (unsigned) (m_location[0] & 0xFFFFu); }
...
struct HltSelRepRBObjTyp {
  ...
  __device__ __host__ unsigned numberOfObjTyp() const { return (unsigned) (m_location[0] & 0xFFFFu); }

  __device__ __host__ unsigned size() const { return (numberOfObjTyp() + 1); }

  __device__ __host__ static unsigned sizeFromPtr(const uint32_t* ptr) { return (unsigned) (ptr[0] & 0xFFFFu) + 1; }
...
struct HltSelRepRBSubstr {
  ...
  __device__ __host__ unsigned numberOfObj() const { return (unsigned) (m_location[0] & 0xFFFFL); }

  __device__ __host__ unsigned allocatedSize() const { return (unsigned) ((m_location[0] & 0xFFFF0000L) >> 16); }

  __device__ __host__ unsigned lenSubstr(unsigned inpt) const { return (unsigned) ((inpt & 0xFFFF) >> 1); }
...
struct HltSelRepRawBank {
  ...
  enum DefaultAllocation { kDefaultAllocation = 4000 };

  unsigned* m_location;

  __device__ __host__ HltSelRepRawBank() {};
  __device__ __host__ HltSelRepRawBank(unsigned* base_pointer, unsigned len = DefaultAllocation::kDefaultAllocation)
  {
    m_location = base_pointer;
  ...
  __device__ __host__ unsigned numberOfSubBanks() const { return (unsigned) (m_location[Header::kSubBankIDs] & 0x7u); }
  ...
  __device__ __host__ unsigned subBankSize(unsigned iBank) const { return (subBankEnd(iBank) - subBankBegin(iBank)); }

  __device__ __host__ unsigned subBankSizeFromID(unsigned idSubBank) { return subBankSize(indexSubBank(idSubBank)); }
  ...
  __device__ __host__ unsigned allocatedSize() const { return m_location[Header::kAllocatedSize]; }
  ...
  __device__ __host__ void push_back(unsigned idSubBank, const unsigned* pSubBank, unsigned sizeSubBank)
  {
    ...
    m_location[Header::kSubBankIDs] = (m_location[Header::kSubBankIDs] & ~0x7u) | (iBank + 1);
