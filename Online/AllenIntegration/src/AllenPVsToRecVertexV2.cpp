...
std::vector<LHCb::Event::v2::RecVertex> AllenPVsToRecVertexV2::operator()(const HostBuffers& host_buffers, const Beamline_t& beamline) const {
  ...
  for ( int i = 0; i < n_pvs; i++ ) {
    ...
    if ( vertex.nTracks >= m_minNumTracksPerVertex && beamlinerho2 < maxVertexRho2 ) { // already checked in Allen PV finder
      ...
      auto& recvertex = recvertexcontainer.emplace_back( position, poscov,
