...
template<>
void SequenceVisitor::set_arguments_size<run_postscale_t>(
  const run_postscale_t& state,
  run_postscale_t::arguments_t arguments,
...
template<>
void SequenceVisitor::visit<run_postscale_t>(
  run_postscale_t& state,
  const run_postscale_t::arguments_t& arguments,
  const RuntimeOptions& runtime_options,
  const Constants& constants,
